...
class RegionUploadSignController extends Controller
{
    public function index(){
        $user = Auth::user();
        
        // Get documents with status "Accept by AMO Region" and not yet signed by Region
        $documents = Document::with(['category', 'area', 'user'])
            ->where('status', 'Accept by AMO Region')
            ->where('is_signed_region', false)
        ...
        return Inertia::render("AMO Region/Upload Sign/page", [
            'auth' => [
                'user' => [
                    'id' => $user->id,
                    'nama' => $user->nama,
    ...
    public function preview($id)
    {
        ...
        if ($document->status !== 'Accept by AMO Region') {
    ...
    public function store(Request $request)
    {
        ...
        if ($document->status !== 'Accept by AMO Region') {
            return redirect()->back()->with('error', 'Only documents with status "Accept by AMO Region" can be signed');
        }
        ...
        try {
            ...
            $document->is_signed_region = true;
            $document->save();
