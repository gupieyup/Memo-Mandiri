...
import { FiUpload, FiHome, FiX } from "react-icons/fi";
import { TbLogout } from "react-icons/tb";

const AreaSidebar = ({ handleLogout, isOpen, onClose }) => {
    const { url } = usePage();

    const menuItems = [
        { href: "/amo-area/home", icon: FiHome, label: "Home" },
        {
            href: "/amo-area/upload-document",
            icon: FiUpload,
    ...
    return (
        <>
            ...
            {isOpen && (
                <div
                    className="fixed inset-0 z-30 bg-gray-900/50 backdrop-blur-sm md:hidden transition-opacity duration-300"
                    onClick={onClose}
                />
            )}

            <aside
                className={`fixed top-0 left-0 z-40 w-72 h-screen transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full sm:translate-x-0"
                    }`}
                aria-label="Sidebar"
            >
                <div className="h-full bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50 border-r border-gray-200 shadow-xl relative">
                    {/* Mobile Close Button */}
                    <button
                        onClick={onClose}
                        className="absolute top-4 right-4 p-2 md:hidden text-gray-500 hover:text-red-500 transition-colors"
                    >
                        <FiX size={24} />
                    </button>
