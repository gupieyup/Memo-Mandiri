...
import { FiUpload, FiHome, FiPenTool, FiX } from "react-icons/fi";
...
const RegionSidebar = ({ handleLogout, isOpen, onClose }) => {
    ...
    const menuItems = [
        { href: "/amo-region/home", icon: FiHome, label: "Home" },
        {
            href: "/amo-region/upload-document",
            icon: FiUpload,
    ...
    return (
        <>
            ...
            {isOpen && (
                <div
                    className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-30 md:hidden transition-opacity duration-300"
                    onClick={onClose}
                />
            )}

            <aside
                className={`fixed top-0 left-0 z-40 w-72 h-screen transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"
                    } md:translate-x-0`}
                aria-label="Sidebar"
            >
                <div className="h-full bg-gradient-to-br from-slate-50 via-blue-50 to-slate-50 border-r border-gray-200 shadow-xl relative">
                    ...
                    <button
                        onClick={onClose}
                        className="absolute top-4 right-4 p-2 rounded-lg hover:bg-gray-200/50 md:hidden transition-colors"
                    >
